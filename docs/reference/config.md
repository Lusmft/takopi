...
### `transports.telegram.images`

Image requests like `нарисуй ...` or `generate image ...` are handled before the normal agent runner flow.

=== "takopi config"

    ```sh
    takopi config set transports.telegram.images.provider openai
    takopi config set transports.telegram.images.model gpt-image-1
    takopi config set transports.telegram.images.api_key "$OPENAI_API_KEY"
    ```
...
| Key | Type | Default | Notes |
|-----|------|---------|-------|
| `enabled` | bool | `true` | Enable Telegram image-intent handling. |
| `provider` | `"stub"`\|`"openai"` | `"stub"` | `stub` returns an SVG placeholder; `openai` calls the OpenAI Images API. |
| `model` | string | `"gpt-image-1"` | Image generation model. |
| `size` | `"1024x1024"`\|`"1024x1536"`\|`"1536x1024"`\|`"auto"` | `"1024x1024"` | Requested image size. |
| `quality` | `"low"`\|`"medium"`\|`"high"`\|`"auto"` | `"auto"` | Provider image quality hint. |
| `output_format` | `"png"`\|`"jpeg"`\|`"webp"` | `"png"` | Filename/MIME hint for generated bytes. |
| `base_url` | string\|null | `null` | Optional OpenAI-compatible base URL. |
| `api_key` | string\|null | `null` | Optional provider API key; OpenAI falls back to `OPENAI_API_KEY`. |
